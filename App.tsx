...
interface AppContextType {
  ...
  activeModal: { leadId: string | null; tab: 'history' | 'tasks' } | null;
  openLeadModal: (leadId: string | null, tab?: 'history' | 'tasks') => void;
  closeLeadModal: () => void;
}
  ...
  const handleFileKeyUpload = async () => {
    try {
      ...
      const [handle] = await window.showOpenFilePicker({
        types: [{ description: 'API Key File', accept: { 'text/plain': ['.txt'], 'application/json': ['.json'] } }],
        multiple: false
      });
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-900 p-6 relative overflow-hidden">
        ...
        <div className="w-full max-w-md z-10 animate-in zoom-in-95 duration-500 text-center">
          ...
          <p className="text-slate-400 font-medium mb-8 leading-relaxed">
            To enable AI-powered lead analysis and automated follow-ups, Nexus needs a Gemini API key.
          </p>
...
const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Global Modal State
  const [activeModal, setActiveModal] = useState<{ leadId: string | null; tab: 'history' | 'tasks' } | null>(null);

  const openLeadModal = (leadId: string | null, tab: 'history' | 'tasks' = 'history') => {
    setActiveModal({ leadId, tab });
  };
  ...
  return (
    <AppContext.Provider value={{ 
      ...
      activeModal, openLeadModal, closeLeadModal
    }}>
      {children}
    </AppContext.Provider>
  );
...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const { state, logout, markNotificationRead, isSynced, searchQuery, setSearchQuery, activeModal, closeLeadModal } = useApp();
  ...
  const leadToEdit = useMemo(() => {
    if (!activeModal || !activeModal.leadId) return undefined;
    return state.leads.find(l => l.id === activeModal.leadId);
  }, [activeModal, state.leads]);
...
const Contacts: React.FC = () => {
  const { filteredLeads, openLeadModal } = useApp();
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => openLeadModal(null)} className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-2xl font-black shadow-lg shadow-blue-200 transition-all active:scale-95">
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredLeads.length === 0 ? (
                ...
                filteredLeads.map(lead => (
                  <tr 
                    key={lead.id} 
                    onDoubleClick={() => openLeadModal(lead.id)}
