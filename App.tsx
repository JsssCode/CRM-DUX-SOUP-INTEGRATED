...
const ApiKeyGuard: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const checkKey = async () => {
      ...
      // 2. Check localStorage for a manually uploaded key
      const savedKey = localStorage.getItem("NEXUS_MANUAL_KEY");
      if (savedKey) {
        // @ts-ignore
      ...
      // 3. Check window.aistudio bridge
      // @ts-ignore
      if (
        window.aistudio &&
  ...
  const handleFileKeyUpload = async () => {
    try {
      // @ts-ignore
      ...
      // If it's a JSON file, try to find an 'apiKey' or 'key' field
      if (file.name.endsWith(".json")) {
        try {
          const parsed = JSON.parse(content);
          key = parsed.apiKey || parsed.key || parsed.API_KEY || key;
        } catch (e) {}
      }

      if (key && key.length > 10) {
        localStorage.setItem("NEXUS_MANUAL_KEY", key);
        // @ts-ignore
        process.env.API_KEY = key;
        setHasKey(true);
      } else {
        alert("The selected file doesn't seem to contain a valid API key.");
      }
    } catch (err: any) {
      ...
      <div className="min-h-screen flex items-center justify-center bg-slate-900 p-6 relative overflow-hidden">
        ...
        <div className="w-full max-w-md z-10 animate-in zoom-in-95 duration-500 text-center">
          ...
          <p className="text-slate-400 font-medium mb-8 leading-relaxed">
            To enable AI-powered lead analysis and automated follow-ups, Nexus
          ...
          <div className="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 p-8 rounded-[2.5rem] shadow-2xl mb-8">
            ...
            <button
              onClick={handleFileKeyUpload}
              className="w-full py-4 bg-slate-700/50 text-slate-300 font-black rounded-2xl border border-slate-600 hover:bg-slate-700 hover:text-white transition-all flex items-center justify-center gap-3"
            >
              <FileCode size={20} />
            ...
            <a
              ...
              className="mt-6 text-[10px] font-black text-slate-500 uppercase tracking-widest hover:text-blue-400 transition-colors flex items-center justify-center gap-2"
            >
...
const AppProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [state, setState] = useState<CRMState>(() => {
      ...
      notifications: [
        {
          id: "n1",
          title: "Welcome",
          message: "Nexus CRM is ready for your sales!",
          type: "success",
          timestamp: new Date().toISOString(),
          read: false,
        },
      ],
      users: [{ id: "u1", name: "Default Agent", role: "Sales Lead" }],
      currentUser: null,
    };
    ...
    try {
      ...
      if (saved) {
        ...
        // CRITICAL FIX: Merge saved data with defaults to prevent "undefined" errors on new properties (like users)
        return {
          ...DEFAULT_STATE,
          ...parsed,
          // Ensure arrays exist even if they weren't in the old saved data
  ...
  const connectLocalFile = async () => {
    try {
      ...
      if (content) {
        const parsed = JSON.parse(content);
        // Merge again here just to be safe with external files
        setState((prev) => ({
          ...prev,
          ...parsed,
          leads: parsed.leads || prev.leads,
          users: parsed.users || prev.users,
          notifications: parsed.notifications || prev.notifications,
        }));
        addNotification(
          "Local Sync Active",
          `Connected to ${handle.name}`,
          "success",
        );
      }
    } catch (err: any) {
  ...
  const addLead = (leadData: any) => {
    ...
    addNotification(
      "Lead Added",
      `${newLead.name} was added to pipeline.`,
      "info",
    );
  };
  ...
  const updateLead = (id: string, updates: Partial<Lead>) => {
    setState((prev) => ({
      ...
      leads: prev.leads.map((l) =>
        l.id === id
          ? { ...l, ...updates, lastActivity: new Date().toISOString() }
          : l,
      ),
    }));
  };
      ...
      <aside className="w-64 bg-slate-900 text-white flex flex-col shadow-xl shrink-0">
        <div className="p-6 border-b border-slate-800">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold tracking-tight uppercase tracking-[0.2em]">
              Nexus
            </h1>
          </div>
        </div>
        ...
        <div className="p-4 space-y-4">
          <div
            className={`p-3 rounded-xl border flex items-center gap-3 transition-all ${isSynced ? "bg-green-500/10 border-green-500/30" : "bg-slate-800/50 border-slate-700"}`}
          >
            <HardDrive
              size={18}
              className={isSynced ? "text-green-400" : "text-slate-500"}
            />
            <span
              className={`text-[10px] font-bold uppercase tracking-widest ${isSynced ? "text-green-400" : "text-slate-500"}`}
            >
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-8 shrink-0 shadow-sm z-10">
          ...
          <div className="flex items-center gap-6">
                    ...
                    {state.notifications.length === 0 ? (
                      ...
                      state.notifications.map((n) => (
                        <div
                          ...
                          className={`p-4 border-b border-slate-100 cursor-pointer hover:bg-slate-50 transition-colors ${!n.read ? "bg-blue-50/50" : ""}`}
                        >
            ...
            <div className="flex items-center gap-3 pl-6 border-l border-slate-200">
              ...
              <img
                src={`https://ui-avatars.com/api/?name=${state.currentUser?.name}&background=0D8ABC&color=fff`}
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-900 p-6 relative overflow-hidden">
      ...
      <div className="w-full max-w-4xl z-10 animate-in zoom-in-95 duration-500">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {(state.users || []).map((user) => (
            ...
            >
              <img
                ...
                className="w-20 h-20 rounded-[1.5rem] mb-6 shadow-lg border-2 border-slate-700 group-hover:border-blue-500 transition-colors"
              />
          ...
          {showAddForm ? (
            <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl animate-in slide-in-from-bottom-4">
              <h3 className="font-black text-slate-900 mb-6 flex items-center gap-2 uppercase tracking-wider text-sm">
                <UserPlus2 size={18} /> New Agent
              </h3>
              <form onSubmit={handleCreate} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-5 py-3 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-blue-500 outline-none text-sm font-bold"
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <button
                    type="submit"
                    className="flex-1 py-3 bg-blue-600 text-white text-sm font-black rounded-2xl shadow-lg shadow-blue-500/20 hover:bg-blue-700 transition-all"
                  >
                    Create
                  </button>
                </div>
        ...
        <p className="mt-12 text-center text-[10px] text-slate-500 uppercase tracking-[0.3em] font-black">
          Local-First Storage • No Password Required
        </p>
      </div>
    </div>
  );
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-500 font-medium text-sm">
            Double-click a row to open profile
          </p>
        </div>
        <button
          onClick={handleAddNew}
          className="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-2xl font-black shadow-lg shadow-blue-200 transition-all active:scale-95"
        >
              ...
              {filteredLeads.length === 0 ? (
                ...
                filteredLeads.map((lead) => (
                  ...
                  >
                    ...
                    <td className="px-8 py-5">
                      <span
                        className={`text-[9px] font-black uppercase tracking-widest flex items-center gap-2 ${lead.source === "Dux-Soup" ? "text-blue-600" : "text-slate-400"}`}
                      >
    ...
    <div className="max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div>
        ...
        <p className="text-slate-500 font-medium">
          Workspace configuration and data security
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200 space-y-6">
          <h3 className="font-black text-slate-900 flex items-center gap-3 uppercase tracking-wider text-sm">
            <HardDrive className="text-blue-600" size={20} /> Data Storage
          </h3>
          <p className="text-sm text-slate-600 leading-relaxed font-medium">
            Nexus CRM uses <strong>Local-First Storage</strong>. All leads,
            tasks, and users are stored in a single JSON file on your machine.
          </p>
          {isSynced ? (
            <div className="p-5 bg-green-50 border border-green-200 rounded-[1.5rem] flex items-center justify-between">
              ...
              <button
                ...
                className="p-2 text-green-700 hover:bg-green-100 rounded-xl transition-all"
              >
                <RefreshCw size={16} />
              </button>
            </div>
            ...
            <button
              onClick={connectLocalFile}
              className="w-full bg-slate-900 text-white font-black py-4.5 rounded-2xl shadow-xl shadow-slate-900/10 hover:bg-slate-800 transition-all flex items-center justify-center gap-3"
            >
              <HardDrive size={20} /> Connect Local Database
            </button>
          )}
        </div>
        ...
        <div className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-200 space-y-6">
          <h3 className="font-black text-slate-900 flex items-center gap-3 uppercase tracking-wider text-sm">
            <Users className="text-indigo-600" size={20} /> Team Members
          </h3>
          <div className="space-y-3 max-h-48 overflow-y-auto custom-scrollbar pr-2">
            {(state.users || []).map((u) => (
              ...
              >
                <img
                  src={`https://ui-avatars.com/api/?name=${u.name}&size=64`}
